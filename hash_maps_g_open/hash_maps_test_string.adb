...
procedure Hash_Maps_Test_String is
   ...
   exception
      when Maps.Full_Map =>
         Ada.Text_IO.Put_Line("EXCEPTION: Full_Map");
   end Do_Put;
   ...
   A_Map : Maps.Map;
   A_Key: ASU.Unbounded_String;
begin
   ATIO.Put_Line("SHOULD RAISE NO_ELEMENT EXCEPTION");
   begin
      A_Key := Maps.Element(Maps.First(A_Map)).Key;
   exception
        when Ex:others =>
            ATIO.Put_Line("EXCEPTION " & Ada.Exceptions.Exception_Name(Ex) &
                        ": " & Ada.Exceptions.Exception_Message(Ex));
   end;

   Do_Delete (A_Map, ASU.To_Unbounded_String("SHOULD SAY NOT FOUND (EMPTY_MAP)"));  -- Does not exist
