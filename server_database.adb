...
package body Server_Database is
    ...
    procedure Register (Buffer: access LLU.Buffer_Type) is
        ...
        Success_Inactive: Boolean;
        Inactive_Date: Ada.Calendar.Time;
    ...
    begin
        ...
        if In_System = False then
            ...
            SD.Inactive_Type_Map.Get(SD.Inactive_Clients, Nick, Inactive_Date, Success_Inactive);
            if Success_Inactive = True then
                SD.Inactive_Type_Map.Delete(SD.Inactive_Clients, Nick, Success_Inactive);
                if Success_Inactive = False then
                    Text_IO.Put_Line("Deleting was unsuccessfull");
                end if;
                Send_All(Buffer, Nick, Server_String, ASU.To_Unbounded_String (ASU.To_String(Nick) & " rejoins the chat"));
            else
